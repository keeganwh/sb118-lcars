...
- **A new shared file means updating the `vercel.json` cache-header list.** Miss it and a fix never reaches anyone's browser. It used to mean updating `api/download.js` too — **it no longer does**, since the offline download is frozen at v4.24 and does not grow a fourth inline. The route fails loudly instead, which is the intended end state.
