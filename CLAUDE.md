...
- **Offline download — FROZEN at v4.24.** _Decided 2026-08-24; the freeze landed 2026-08-26._ `api/download.js` re-inlines the three files into one self-contained `LCARS.html` on demand. **v4.24 is the last version it carries** — verified booting clean from `file://` at `21d8aec` (v4.24 plus the changes pending for the next version). It is **not to be extended**: features that require the server (Joint Posts, share links, accounts, syncing, anything HQ) are deliberately not carried into it. Do not contort a feature to keep the inlined build working, and do not add a fourth inline to it — **the route now refuses to build if a new shared file lands**, which is deliberate. That refusal is the signal to start "LCARS Offline", not a bug to fix by growing the route. `OFFLINE_FROZEN_VERSION` at the top of `lcars.js` is what the Settings copy states — it deliberately does **not** follow `APP_VERSION`, so never bump it on a version bump. If a genuinely offline app is wanted later, the agreed answer is a purpose-built, pared-back **"LCARS Offline"**, not a bigger inliner — **deferred at low priority to `ROADMAP.md` → Batch 12.** The freeze work in Batch 3 is deliberately incomplete and shelved.
...
8. **Don't extend the offline download, and add no new features to LCARS Offline.** It is frozen at v4.24 (see above). **Standing rule from 2026-08-26: fixes and tweaks yes, new features no** — regardless of what lands online. The freeze work is *deliberately incomplete and shelved at low priority*; the pared-back "LCARS Offline" follow-up is deferred to `ROADMAP.md` → Batch 12. Don't start it without being asked. A new shared file still needs the `vercel.json` cache-header list updated, but `api/download.js` no longer grows — a new shared file does **not** get inlined into it.
...
- **A new shared file means updating the `vercel.json` cache-header list.** Miss it and a fix never reaches anyone's browser. It used to mean updating `api/download.js` too — **it no longer does**, since the offline download is frozen at v4.24 and does not grow a fourth inline. Instead the route **refuses to build**, on purpose: without that check it would have built cleanly and shipped an offline copy silently missing the new file. Expect the download to start failing the day a fourth shared file lands, and treat it as the prompt to build "LCARS Offline" (Batch 12) rather than as a regression.
