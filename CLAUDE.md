...
- **An assertion can confirm something that is visibly broken, and it did three times in one session.** Once it measured an element that never rendered; once every colour check passed on a dashboard header whose title had collapsed over two lines; and once a "press Back four times" check passed while clicking a button that did not exist, because `.catch()` swallowed the failure and the unchanged step counter read as a pass. **Assert that the thing MOVED, not that it ended up somewhere — and screenshot the result as well as measuring it.**
