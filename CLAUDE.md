...
8. **Don't extend the offline download.** It is frozen at v4.24 (see above). A new shared file still needs the `vercel.json` cache-header list updated, but `api/download.js` no longer grows — a new shared file does **not** get inlined into it.
...
- **A new shared file means updating the `vercel.json` cache-header list.** Miss it and a fix never reaches anyone's browser. It used to mean updating `api/download.js` too — **it no longer does**, since the offline download is frozen at v4.24 and does not grow a fourth inline. Instead the route **refuses to build**, on purpose: without that check it would have built cleanly and shipped an offline copy silently missing the new file. Expect the download to start failing the day a fourth shared file lands, and treat it as the prompt to build "LCARS Lite" rather than as a regression.
